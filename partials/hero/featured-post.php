...
<!-- hero: featured post -->
<section
    class="hero hero_featured-post relative z-1 <?=(!empty($hero['featured_post']) ? 'has-post' : '')?> <?=(!empty($hero['bg']) ? 'has-bg' : '')?>">
    ...
    <div
        class="hero_featured-post__container relative z-1 container layout-grid items-start gap-24 <?=(!empty($hero['bg']) ? 'py-48 lg:py-96 xl:py-120' : 'mt-48 lg:mt-96 xl:mt-120')?> ">

        <div
            class="hero_featured-post__content col-4 <?=(!empty($hero['featured_post']) ? 'lg:col-8 xl:col-5 self-center' : 'lg:col-6 lg:start-col-4')?> max-w-710 grid gap-12 lg:gap-24  <?=(!empty($hero['featured_post']) ? '' : 'text-center')?> <?=(!empty($hero['bg']) ? '' : '')?>">
        ...
        <a href="<?=get_permalink($featured->ID)?>"
            class="hero_featured-post__post relative z-1 col-4 lg:col-12 xl:col-6 lg:start-col-7 flex flex-col items-start gap-12 ar-4:3 p-24 lg:p-48 text-white bg-black">
            <?php echo core::get_custom_srcset(
					array(
						'lazy'=>false,
