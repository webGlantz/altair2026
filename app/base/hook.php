...
abstract class hook {
	...
	public static function init() : bool {
		...
		foreach (static::ACTIONS as $hook=>$callbacks) {
			foreach ($callbacks as $callback=>$args) {
				$args = common::parse_args($args, static::ACTION_OPTIONS);
				\add_action(
					$hook,
		...
		// Remove actions.
		foreach (static::REMOVE as $hook=>$callbacks) {
			foreach ($callbacks as $callback=>$args) {
				$args = common::parse_args($args, static::ACTION_OPTIONS);
		...
		foreach (static::FILTERS as $hook=>$callbacks) {
			foreach ($callbacks as $callback=>$args) {
				$args = common::parse_args($args, static::FILTER_OPTIONS);
				\add_filter(
					$hook,
					array($class, $callback),
					$args['priority'],
	 ...
	 * @return void Nothing.
	 */
	protected static function special_init() : void {
		return;
	}
